...
float UcdObject::getPressureSphereValue( unsigned int index )
{
	...
	if( !is_skipped_to_pressure ) 
	{
		...
		for ( unsigned int i = 0; i < m_ncomponents; i++ )
		{
			...
			::SkipValue<int>( m_ifs );
			::SkipValue<int>( m_ifs );
			::SkipValue<float>( m_ifs ); // Component ID and properties.
		}
...
// kvs::ValueArray<float> UcdObject::toPressureArray( float fraction )
// {
//     this->skipToValues();
...
//     kvs::ValueArray<float> pressur_ori = ::GetValues<float>( m_ifs, m_total_nnodes );
