//
//  ParticleBasedRenderer.cpp
//
//
//  Created by Kun Zhao on 2015-11-20 11:47:57.
//
//
...
// Mode
void ParticleBasedRenderer::enableDensityMode()
{
    static_cast<Engine&>( engine() ).enableDensityMode();
}

void ParticleBasedRenderer::disableDensityMode()
{
    static_cast<Engine&>( engine() ).disableDensityMode();
}

void ParticleBasedRenderer::enableSizesMode()
{
    static_cast<Engine&>( engine() ).enableSizesMode();
}

void ParticleBasedRenderer::disableSizesMode()
{
    static_cast<Engine&>( engine() ).disableSizesMode();
}
...
ParticleBasedRenderer::Engine::Engine():
    ...
    m_base_opacity( 0.3 ),
    m_enable_density( false ),
    m_enable_sizes( false )
{
}
...
void ParticleBasedRenderer::Engine::setup( kvs::ObjectBase* object, kvs::Camera* camera, kvs::Light* light )
{
    ...
    if( m_enable_sizes ) m_size_location = m_shader_program.attributeLocation( "size" );
}
...
void ParticleBasedRenderer::Engine::draw( kvs::ObjectBase* object, kvs::Camera* camera, kvs::Light* light )
{
    ...
    if( m_enable_density ) this->initialize_density_volume_texture();
    ...
    kvs::Texture::Binder unit3( m_density_texture, 2 );
    {
        ...
        const size_t size_offset = value_offset + value_size;
...
void ParticleBasedRenderer::Engine::create_shader_program()
{
    ...
    if( m_enable_density )
    {
        vert.define( "ENABLE_PARTICLE_DENSITY" );
    }
...
void ParticleBasedRenderer::Engine::create_buffer_object( const kun::PointObject* point )
{
    ...
    for ( size_t i = 0; i < repetitionLevel(); i++ )
    {
        ...
        const size_t normal_size = m_has_normal ? count * sizeof(kvs::Real32) * 3 : 0;
        const size_t value_size = count * sizeof( kvs::Real32 );
        const size_t size_size = m_enable_sizes ? ( count * sizeof( kvs::Real32 ) ) : 0;
        
        const size_t byte_size = coord_size + normal_size + value_size + size_size;
        ...
        if( m_enable_sizes )
        {   
            m_vbo[i].load( size_size, sizes.data() + first, coord_size + normal_size + value_size );
        }
...
void ParticleBasedRenderer::Engine::initialize_density_volume_texture()
{
