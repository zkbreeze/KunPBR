...
//
//
//  Created by Kun Zhao on 2015-11-25 20:38:09.
//
//
...
kvs::ValueArray<kvs::Real32> VertexNormals( const kvs::PolygonObject* polygon )
{
    ...
    case kvs::PolygonObject::PolygonNormal:
    {
    	...
    	const unsigned int* pconnections = polygon->connections().data();
    	const float* ori_normals = polygon->normals().data();
    	
    	// Initialize the normal vectors
    	normals.allocate( nvertices * 3 );
    	float* pnormals = normals.data();
    	for( size_t i = 0; i < nvertices * 3; i++ ) pnormals[i] = 0.0;

    	// Calculate the normal verctors for each vertex
    	for( size_t i = 0; i < npolygons; i++ )
    	{
    		pnormals[pconnections[i * 3] * 3] += ori_normals[i * 3];
    		pnormals[pconnections[i * 3] * 3 + 1] += ori_normals[i * 3 + 1];
    		pnormals[pconnections[i * 3] * 3 + 2] += ori_normals[i * 3 + 2];

    		pnormals[pconnections[i * 3 + 1] * 3] += ori_normals[i * 3];
    		pnormals[pconnections[i * 3 + 1] * 3 + 1] += ori_normals[i * 3 + 1];
    		pnormals[pconnections[i * 3 + 1] * 3 + 2] += ori_normals[i * 3 + 2];

    		pnormals[pconnections[i * 3 + 2] * 3] += ori_normals[i * 3];
    		pnormals[pconnections[i * 3 + 2] * 3 + 1] += ori_normals[i * 3 + 1];
    		pnormals[pconnections[i * 3 + 2] * 3 + 2] += ori_normals[i * 3 + 2];
    	}
        break;
    }
    default: break;
